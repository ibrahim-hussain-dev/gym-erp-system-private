...
def send_welcome_email(doc, method=None):
    ...
    """.format(full_name=doc.full_name or "Member", member_id=doc.name)
...
def send_freeze_email(subscription_name):
    ...
    full_name = get_member_full_name(sub.member)
    ...
    """.format(full_name=full_name or "Member", plan=plan_name, start=sub.freeze_start_date, end=sub.freeze_end_date)
...
def send_auto_renew_email(member, plan_name, new_invoice, new_end_date):
    ...
    full_name = get_member_full_name(member)
    ...
    """.format(full_name=full_name or "Member", plan=plan_name, invoice=new_invoice, end_date=new_end_date)
    safe_send(email, subject, message)
...
def send_noshow_email(slot_name):
    ...
    full_name = get_member_full_name(slot.member)
    ...
    """.format(full_name=full_name or "Member", date=slot.date, time=slot.start_time, trainer=trainer_name)
    safe_send(email, subject, message)
...
def send_expiry_reminders():
    ...
    for sub in subs:
        ...
        """.format(full_name=full_name or "Member", plan=plan_name, end_date=sub.end_date)
        safe_send(email, subject, message)
...
def send_invoice_due_reminders():
    ...
    for inv in invoices:
        ...
        full_name = get_member_full_name(inv.gym_member)
        ...
        """.format(full_name=full_name or "Member", invoice=inv.name, amount=inv.outstanding_amount, due_date=inv.due_date)
        safe_send(email, subject, message)
